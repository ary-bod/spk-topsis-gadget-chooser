...
if ($_GET) {
  $id = $_GET['id'];
  $result_data = query("SELECT * FROM gadgets WHERE id=$id")[0];
}
...
if ($_POST) {
  ...
  $update_query = "UPDATE gadgets SET name='$name', price='$price', memory='$memory', storage='$storage', processor='$processor', 
  camera_front='$camera_front', camera_back='$camera_back', battery='$battery',
  price_score='$price_score', memory_score='$memory_score', storage_score='$storage_score', processor_score='$processor_score', 
  camera_front_score='$camera_front_score', camera_back_score='$camera_back_score', battery_score='$battery_score'
  WHERE id=$id";
}
...
<body>
  ...
  <div class="px-4 py-5 my-5 text-center">
    <h1 class="display-5 fw-bold">Our Gadget List - Edit Gadget</h1>
  ...
  <div class="px-4 py-5 my-5">
    <div class="container">
      <?php if ($_POST) : ?>
        <?php if (mysqli_query($connection, $update_query)) : ?>
          <?php $result_data = query("SELECT * FROM gadgets WHERE id=$id")[0]; ?>
          <div class="alert alert-success">
            Gadget edit successfully!
          </div>
        <?php else : ?>
          <div class="alert alert-warning">
            Gadget failed to edit! Error: <?= mysqli_errno($connection); ?>
          </div>
        <?php endif; ?>
      <?php endif; ?>
      <form method="POST">
        <div class="row">
          <div class="col-md-12">
            <div class="form-group mb-3">
              ...
              <input name="name" type="text" class="form-control" id="name" value="<?= $result_data['name'] ?>" required>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group mb-3">
              <label class="form-label" for="price"><span class="text-danger">* </span>Gadget Price (Rupiah)</label>
              <input name="price" type="number" class="form-control" id="price" value="<?= $result_data['price'] ?>" required>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group mb-3">
              <label class="form-label" for="memory"><span class="text-danger">* </span>Gadget Memory (GB)</label>
              <input name="memory" type="number" class="form-control" id="memory" value="<?= $result_data['memory'] ?>" required>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group mb-3">
              <label class="form-label" for="storage"><span class="text-danger">* </span>Gadget Storage (GB)</label>
              <input name="storage" type="number" class="form-control" id="storage" value="<?= $result_data['storage'] ?>" required>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group mb-3">
              <label class="form-label" for="processor"><span class="text-danger">* </span>Gadget Processor</label>
              <input name="processor" type="number" class="form-control" id="processor" value="<?= $result_data['processor'] ?>" required>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group mb-3">
              <label class="form-label" for="camera_front"><span class="text-danger">* </span>Gadget Camera Front (MP)</label>
              <input name="camera_front" type="number" class="form-control" id="camera_front" value="<?= $result_data['camera_front'] ?>" required>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group mb-3">
              <label class="form-label" for="camera_back"><span class="text-danger">* </span>Gadget Camera Back (MP)</label>
              <input name="camera_back" type="number" class="form-control" id="camera_back" value="<?= $result_data['camera_back'] ?>" required>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group mb-3">
              <label class="form-label" for="battery"><span class="text-danger">* </span>Gadget Battery (mAh)</label>
              <input name="battery" type="number" class="form-control" id="battery" value="<?= $result_data['battery'] ?>" required>
            </div>
          </div>
        </div>
        <button type="submit" class="btn btn-primary mt-3">Edit Gadget</button>
      </form>
    </div>
  </div>
